...
var CmdWeb = cli.Command{
	Name:        "web",
	Usage:       "start plumbing ssh service",
	...
	Flags: []cli.Flag{
		cli.StringFlag{
			Name:  "address",
			Value: "0.0.0.0",
			Usage: "ssh service listen ip, default is 0.0.0.0;",
		},
		cli.IntFlag{
			Name:  "port",
			Value: 22,
			Usage: "ssh service listen at port 22;",
		},
	},
}
...
func runWeb(c *cli.Context) {
    ...
    address, port := getlistenAddress(c)
...
func getlistenAddress(c *cli.Context) (string, string) {
    ...
    listenaddr := fmt.Sprintf("%s:%s", c.String("address"), port)
